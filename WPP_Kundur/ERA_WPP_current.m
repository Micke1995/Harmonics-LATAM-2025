...
% Ts = 1/Fs;
figure;
Iabc1 = signals(end-12*No:end,22:24);
plot(t(end-12*No:end), Iabc1)
xlabel('Time (s)')
ylabel('Current (pu)')
legend('i_a', 'i_b', 'i_c', 'Orientation', 'Horizontal')
grid on
set(gca, 'LooseInset', [0,0,0,0]);
%
Iabc = signals(end-12*No:end,22);
N = length(Iabc)-1;
fun = Iabc(1:N)';
fun2 = Iabc(1:N+1)';
k = 12;
...
amp_era = [todo2(6,2) todo2(6,2) todo2(2,2) todo2(2,2) todo2(1,2) todo2(1,2) todo2(5,2) todo2(5,2) todo2(4,2) todo2(4,2) todo2(3,2) todo2(3,2)];
...
figure;
harmonics_Ia = [harmonics_FFT harmonics_ERA];
bar(harmonics_Ia)
...
I_fft_Ia = 0;
for i = 1:6
    I_era_Ia = todo2(i,2).*cos(todo2(i,1)*2*pi*t(1:N) + todo2(i,3) ) + I_era_Ia;
    I_fft_Ia = spec2(i+6).*cos( fre_fft(i+6)*2*pi*t(1:N) + aspec2(i+6) ) + I_fft_Ia;
end

figure;
Ia = fun';
ti = t(1:N);
plot(t(1:N), Ia, t(1:N), I_fft_Ia, '-.', t(1:N), I_era_Ia, '--')
ylabel('Current (A)')
xlabel('Time (s)')
...
RMSE_FFT = sqrt(mean((fun' - I_fft_Ia).^2)) 
RMSE_ERA = sqrt(mean((fun' - I_era_Ia).^2))
a = norm(fun',2)^2;
b_fft = norm(fun,2)^2 - norm(I_fft_Ia,2)^2;
b_era = norm(fun,2)^2 - norm(I_era_Ia,2)^2;
SNR_FFT = 10*log10(a/b_fft);
SNR_ERA = 10*log10(a/b_era);
SSE_FFT = norm((fun' - I_fft_Ia),2)^2;
% SSE_FFT = sum((fun' - I_fft).^2);
SSE_ERA = sum((fun' - I_era_Ia).^2);
MSE_FFT = (1/N)*SSE_FFT;
MSE_ERA = (1/N)*SSE_ERA;
...
figure;
subaxis(2, 1, 1, 'Spacing', 0.1, 'MarginRight', 0.02, 'MarginLeft', 0.09, 'SpacingVert', 0.08, 'MarginTop', 0.04, 'MarginBottom', 0.2)
Zp = pinv(ZZ);
for kk = 1:k
    ir = Zp(kk,:);
...
subaxis(2, 1, 2, 'Spacing', 0.1, 'MarginRight', 0.02, 'MarginLeft', 0.09, 'SpacingVert', 0.08, 'MarginTop', 0.04, 'MarginBottom', 0.2)
plot(fn, spec);
xlim([-12 12])
% ylabel('Magnitude')
xlabel('Normalised Frequency u=f_1T_1')
